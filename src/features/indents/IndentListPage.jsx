...
const IndentListPage = () => {
  ...
  const getCount = (status) => {
    if (status === 'All') return indents.length;
    return indents.filter(i => i.status === status).length;
  };

  const filteredIndents = filter === 'All' 
    ? indents 
    : indents.filter(i => i.status === filter);

  const columns = [
    { field: 'indentCode', headerName: 'Indent Code', flex: 1 },
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box>
          <IconButton onClick={() => navigate(`/indents/${params.row.id}`)} size="small" color="primary">
            <VisibilityIcon />
          </IconButton>
          {params.row.status === 'Draft' && (
            <IconButton onClick={() => navigate(`/indents/${params.row.id}/edit`)} size="small" color="secondary">
              <EditIcon />
            </IconButton>
          )}
        </Box>
