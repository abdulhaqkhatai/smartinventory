...
import { Dialog, DialogTitle, DialogContent, DialogActions, Button, TextField, Grid, MenuItem, Typography, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Paper } from '@mui/material';
...
const ReturnFormDialog = ({ open, onClose }) => {
  ...
  const handleItemChange = (index, field, value) => {
    ...
    if (field === 'returnQty') {
       newItems[index][field] = Math.min(Number(value) || 0, newItems[index].issuedQty);
    } else {
  ...
  const handleSubmit = (e) => {
    ...
    if (returningItems.length === 0) {
      enqueueSnackbar('Please specify at least one item to return.', { variant: 'error' });
      return;
    }

    const newReturn = {
      id: generateId(),
      code: `RTN-${dayjs().format('YYYY')}-${Math.floor(Math.random()*1000).toString().padStart(3,'0')}`,
      date: formData.date,
      returnedBy: formData.returnedBy,
      department: formData.department,
      receivedBy: 'Rajesh Kumar', // mock current user
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <form onSubmit={handleSubmit}>
        <DialogContent dividers>
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={12} sm={6}>
              ...
              >
                {issues.map(iss => (
                  <MenuItem key={iss.id} value={iss.code}>{iss.code} - {iss.issuedTo}</MenuItem>
                ))}
              </TextField>
            </Grid>
            <Grid item xs={12} sm={6}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField fullWidth label="Returned By" value={formData.returnedBy} disabled />
            </Grid>
            <Grid item xs={12} sm={6}>
        ...
        <DialogActions>
          ...
          <Button type="submit" variant="contained" color="secondary" disabled={items.length === 0}>Record Return</Button>
        </DialogActions>
      </form>
    </Dialog>
  );
